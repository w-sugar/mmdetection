...
class BBoxHead(BaseModule):
    ...
    def get_bboxes(self,
        ...
        if self.use_wordtree_cls != 'WordTreeFocalLoss':
            ...
            scores = F.sigmoid(cls_score) if cls_score is not None else None
            scores_others_bg = torch.gather(cls_score, -1, torch.tensor([10, 16], device=cls_score.device).repeat(scores.shape[0], 1))
            scores_11_16 = scores[:, 11:16].repeat(1, 2)
            scores_11_16 = scores_11_16.index_select(-1, torch.tensor([0,5,1,2,7,3,4,9,8,6], device=cls_score.device))
            scores_0_10 = cls_score[:, :10]
            scores_0_10 = torch.exp(scores_0_10) * scores_11_16
            scores = torch.cat([scores_0_10, torch.exp(scores_others_bg)], 1)
            # scores = F.softmax(scores, dim=-1)
            den = torch.sum(scores, 1, keepdim=True)
            scores = scores / den
